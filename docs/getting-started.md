...
> **Toutes les commandes `./bin/shss ...` de ce document supposent que
...
> shell-integration/shss.bash` fait dans ton `~/.bashrc` (section 7),
> tu n'as par contre plus besoin d'être dans ce dossier ni de taper
> `./bin/shss` du tout : `Ctrl-G` marche depuis n'importe où.
...
2. shss repère ces blocs par une simple regex
   (`src/shss/tags.py`).
...
- **À l'exécution complète de la ligne** — dans le REPL `./bin/shss` ou
...
- **À la demande, via `Ctrl-G`** — résout la balise en train d'être tapée,
  ...
  shss que directement dans ta propre console bash (voir section 7).
...
`/home/jfk/shss-llm-formats.md` (notes perso, hors dépôt).
...
```bash
./.venv/bin/python -c "from shss.llm import discover_gguf_path; print(discover_gguf_path())"
```
...
- Si ça lève `FileNotFoundError` → soit `ollama pull qwen2.5-coder:1.5b-base`
  ...
  ```bash
  export SHSS_MODEL_PATH=/chemin/vers/qwen2.5-coder-1.5b-base.gguf
  ```
...
```bash
./bin/shss -c '#@ affiche la date du jour @#'
./bin/shss -c 'ls #@ trie par taille @#'
./bin/shss -c '#@ liste les fichiers python dans src/ @#'
```
...
Puis, à l'invite `shss:<dossier-courant>$` :

```text
shss:/home/jfk/git/dev/shss$ ls #@ affiche aussi les fichiers caches @#
→ ls -la
...
shss:/home/jfk/git/dev/shss$ cd src
shss:/home/jfk/git/dev/shss/src$ #@ liste les fichiers python ici @#
→ find . -iname "*.py"
...
shss:/home/jfk/git/dev/shss/src$ exit
```
...
## 7. Intégration dans ta console bash normale (sans lancer ./bin/shss)
...
shss affiche ce qu'il a généré (`-S`), puis l'applique directement — la
...
shss ne recevait aucune touche tant que la fonction tournait (seul
Ctrl-C débloquait). C'est un piège documenté de bash (`bind -x` + `read`
interactif), pas un bug shss, et rien ne garantit qu'il ne se
...
Ce mécanisme appelle `bin/shss-resolve-inline` à chaque `Ctrl-G` (donc
...
```bash
./bin/shss -c '#@ formate le fichier /tmp/dede.txt en json et écris le résultat dans /tmp/result.json @#'
...
(vérifié : `time ./bin/shss -c '#@ models @#'` ≈ 0,07s, pas de
chargement du modèle) :

```bash
./bin/shss -c '#@ models @#'                # liste les modèles, indique l'actif
...
```bash
./bin/shss -c '#@ model download 3b @#'   # télécharge
./bin/shss -c '#@ model 3b @#'             # puis l'active (trouve le fichier)
```
...
session, via `SHSS_MODEL_TAG` ou `#@ model <tag> @#`) :

- `sudo ./bin/shss -c '#@ model download 3b @#'` → télécharge dans
  `/opt/shss/models/`, un seul téléchargement pour **tous** les
  utilisateurs de la machine (cohérent avec l'installation via le
  paquet Debian).
- Sans `sudo`, un utilisateur normal télécharge dans son propre
  `~/.shss/models/` (il ne peut pas écrire dans `/opt/shss/`).
...
Ensuite, dans une console où `shell-integration/shss.bash` est
sourcé, `Ctrl-Y` ouvre la liste de tous les modèles Ollama présents
(`shss --list-models` en coulisses, piped dans `fzf`). Le choix
devient actif pour le reste de la session de terminal — `export
SHSS_MODEL_NAME`/`SHSS_MODEL_TAG` est fait directement dans le shell
...
`shell-integration/shss.bash` (`bind -x '"\C-y": shss_pick_model'`)
si tu t'en sers.

## 10. Le code : qui fait quoi

```text
bin/shss                 point d'entrée bash du REPL
bin/shss-resolve-inline  point d'entrée bash pour le Ctrl-G "natif" dans ta console
shell-integration/
  shss.bash              bind -x "\C-g" (résolution) + "\C-y" (fzf), à sourcer
src/shss/
...
- **`bin/shss`** — script bash minimal. Cherche `.venv/bin/python` (sinon
  se rabat sur `python3` système), met `src/` dans `PYTHONPATH`, lance
  `python -m shss.cli`.
- **`bin/shss-resolve-inline`** — même principe, mais lance
...
- **`src/shss/shell.py`** — `PersistentShell` : un seul processus
...
- **`src/shss/cli.py`** — assemble tout : `build_parser()` (argparse,
...
- **`src/shss/inline.py`** — variante non-interactive de la résolution
  ...
  pour un script). `shell-integration/shss.bash` lit ces trois parties
  ...
  minimal, sans rapport avec shss, même lu depuis `/dev/tty` — ne
  recevait aucune touche pendant qu'une fonction `bind -x` tournait (sauf
  Ctrl-C). Piège documenté de bash, pas un bug shss, mais pas assez
...
| --- | --- | --- |
| `SHSS_MODEL_PATH` | Force le chemin exact du `.gguf`, court-circuite la recherche automatique | (aucun) |
...
supprimer cette entrée dans `_KNOWN_OLLAMA_DIRS` (`src/shss/llm.py`),
ou toujours passer par `SHSS_MODEL_PATH`.
